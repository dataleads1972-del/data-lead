...
export class XAIProvider implements AIProvider {
  ...
  async analyzeLead(input: LeadAnalysisInput): Promise<LeadAnalysisResult> {
    ...
    if (!apiKey) {
      throw new Error("XAI_API_KEY environment variable is not configured on the server.");
    }

    const model = input.model || "grok-beta";
...
Metadata: ${JSON.stringify(input.record.metadata || {})}
    ...
    if (!response.ok) {
      ...
      throw new Error(`xAI Provider Error (${response.status}): ${errText}`);
    }

    const data = await response.json();
    const rawContent = data.choices?.[0]?.message?.content;
    if (!rawContent) {
      throw new Error("xAI returned an empty response content.");
    }
